...
let
  ...
  nixos-rebuild-fw3 = pkgs.writeShellScriptBin "nixos-rebuild-fw3" ''
    [[ "$PWD" != "/root" ]] && exit 1
    ACTION=''${1:-boot}
    rm -fr dotfiles/
    cp -r /home/me/src/me/dotfiles .
    nixos-rebuild $ACTION --flake /root/dotfiles#fw3 --print-build-logs --log-format bar-with-logs
  '';
...

{
  ...
  users.users.dev = {
    isNormalUser = true;
    uid = 1001;
    openssh.authorizedKeys.keyFiles = [ config.me.private.keys.me-fw3 ];
  };
